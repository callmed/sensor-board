...
class SensorBoard:
    def __init__(self, node=None,
        ...
        self.sensor_temperature = sensors.SensorDHT22(pin=self.pin_temp,
            name="Env.-Sensor", node=self.node, simulation=self._simulation)
        self.sensor_light = sensors.SensorLight(pin=self.pin_light, name="Light-Sensor",
            node=self.node, simulation=self._simulation)
    ...
    def read(self, threaded=True):
        ...
        thread_sensor_temp = threading.Thread(target=self.sensor_temperature.read())
        threads.append(thread_sensor_temp)
        thread_sensor_light = threading.Thread(target=self.sensor_light.read())
...
#                    f"Temperature: {temperature}, Humidity: {humidity}")
    ...
    def store(self, value_return=False):
        ...
        self._data_storage.append(self._last_reading + (self._last_timestamp, ))
        if value_return:
    ...
    def logo(self):
        image = Image.open("./data/logo.ppm").convert("1")
    ...
    def display(self):
        ...
        self.draw.text((x, top+8), f"T:{value_temp}°C H:{value_humi}%", font=font, fill=255)
    ...
    def write_to_db(self) -> bool:
        ...
        if self._last_reading is not None and self._database is not None:
            try:
                new_measurement = MeasurementModel(sensor_id=self.id,
            ...
            logger.warning(f"Sensor <self.name>: Could not write data to database. No database connection!")
            return False

    def average(self, consider_last=5):
        """ Return the average value of 'consider_last' available measurements in database. """
        query = f"""SELECT avg(temperature), avg(humidity), avg(pressure) FROM (
                    SELECT * FROM measurements ORDER by id DESC LIMIT {consider_last})"""
        if self._last_reading is not None and self._database is not None:
