...
dt = np.dtype=[('temperature', 'f4'), ('humidity', 'f4'), ('timestamp', 'datetime64[ms]')]


def read_data_from_db(conn,table,**kwargs):
    """ Read data from database, converts it into numpy-array and returns it."""
    data_database = []
...
def plot_data_multiplots(data):
    plt.subplot(2,1,1)    # contrained_layout resizes main window to fit all labels
...
def plot_data(data):
    fig, ax = plt.subplots(constrained_layout=False)    # contrained_layout resizes main window to fit all labels
    fig.suptitle("This is a figure sub-title")

    ax.plot_date(data["timestamp"], data["temperature"])
    ax.set_ylim(np.amin(data["temperature"])-10, np.mean(data["temperature"])+10)
...
def plot_data2(data):
    ...
    logger.debug(f"Date conversion started with from {date_start} to " \
                f"{date_end} with steps of {date_delta} / total: {len(dates)}")
    ...
    ax.xaxis.set_minor_locator(mdates.MinuteLocator(np.arange(0, 60, 6)))   # every 6 minutes one tick
